---
title: "Extended Rules and Scheduled Events"
author: "Simon Frost"
---
...
`ReproductionRule` / `MortalityRule` / `PulseRelease` / `SingleDayRelease`
patterns. This vignette covers the **extended** rule and event types
introduced for ecological interactions, conditional triggers, and spatial
dispersal:
...
response (`HollingTypeII`, `HollingTypeIII`, or `FraserGilbertResponse`).
...
`apply_dispersal!(rule, grid, w, day, p)` is the per-day dispersal driver
called by the spatial solver loop.
...
- Extended scheduled events: `ConditionalRelease`,
  `WeatherConditionalEvent`, `SprayEvent`. All `<: AbstractScheduledEvent`.
- Spatial dispersal: `SpatialGrid` + `DispersalRule` + `apply_dispersal!`.
