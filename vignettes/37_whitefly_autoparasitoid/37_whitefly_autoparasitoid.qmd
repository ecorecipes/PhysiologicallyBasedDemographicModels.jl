---
title: "Cotton-Whitefly-Autoparasitoid Dynamics"
subtitle: "Heteronomous Hyperparasitism in Biological Control"
...
Mills & Gutierrez [-@Mills1996Whitefly] developed a tritrophic
physiologically based demographic model (PBDM) for the
cotton–whitefly–parasitoid system that captures three distinct aphelinid
biologies:
...
Schreiber, Mills & Gutierrez [-@Schreiber2001Autoparasitoid] extended
this analysis with stage-structured ODE models to show that obligate
autoparasitism always stabilizes host–parasitoid interactions, while
facultative autoparasitoids can displace primary parasitoids but may
provide inferior host suppression.

This vignette implements the tritrophic cotton–whitefly–autoparasitoid
system using the `PhysiologicallyBasedDemographicModels.jl` API, focusing
on the interplay between parasitoid biology and whitefly suppression.
...
3. **Parasitoids** — three aphelinid species with overlapping generations,
   each with immature and adult stages developing above 13.5 °C.
...
(Gutierrez–Baumgärtner) functional response [@Mills1996Whitefly, Eq. A1–A2]:
...
proportionally to their individual attack rates $\mu_j = 1 - s_j$
[@Mills1996Whitefly, Appendix].
...
Parameters are drawn from Table 1 of Mills & Gutierrez [-@Mills1996Whitefly]
and Table 1 of Schreiber et al. [-@Schreiber2001Autoparasitoid].
...
rises; when scarce, males dominate through hyperparasitism
[@Mills1996Whitefly, Fig. 4].
...
negative exponential, consistent with the yield–pest curve in
Mills & Gutierrez [-@Mills1996Whitefly, Fig. 5]:
$B = 33.5(1 - 10^{-2.87 + 0.55 \cdot \log_{10} Y})$
where $Y$ is cumulative whitefly abundance.
...
Following Schreiber et al. [-@Schreiber2001Autoparasitoid], obligate
autoparasitoids stabilize the equilibrium because male production is
self-limiting — more females means more secondary hosts for males,
creating a negative feedback loop. We illustrate this by comparing the
coefficient of variation in whitefly abundance across scenarios.
...
|:----------|:------|:-------|
| **Temperature thresholds** | | |
| Cotton development | 12 °C | @Mills1996Whitefly, Table 1 |
...
| **Sex ratios** | | |
| P₁ (primary): fixed | 0.50 | @Mills1996Whitefly |
| P₂ (obligate auto.) | density-dependent | @Schreiber2001Autoparasitoid |
| P₃ (facultative auto.) | density-dependent | @Schreiber2001Autoparasitoid |
| **Initialization** | | |
...
2. **Obligate autoparasitoids stabilize dynamics** but may provide
   ...
   [@Schreiber2001Autoparasitoid, Theorem 1].
