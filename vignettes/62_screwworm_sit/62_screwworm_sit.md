...
``` julia
"Concave reproduction temperature scalar (Eq. 1 in paper)."
function ϕT(T; TL = 14.5, TU = 43.5, Topt = 28.0)
    (T < TL || T > TU) && return 0.0
    # symmetric concave triangle peaking at Topt scaled to 1.0
    if T ≤ Topt
        return (T - TL) / (Topt - TL)
    else
        return (TU - T) / (TU - Topt)
