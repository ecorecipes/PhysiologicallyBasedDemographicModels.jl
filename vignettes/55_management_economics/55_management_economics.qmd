---
title: "Management Optimisation and Economics"
author: "Simon Frost"
---

## Overview

The optimal-control surface in `PhysiologicallyBasedDemographicModels.jl`
ships:
...
`AbstractRevenueFunction`, `AbstractDamageFunction`, with
`FixedCost` / `VariableCost` as the primary cost-function concretes.
...
`SITRelease` packages the parameters for a sterile-insect release programme
and `fertile_mating_fraction` returns the fraction of matings that remain
fertile under continued release.
...
- Economics surface: `AbstractCostFunction` (`FixedCost`, `VariableCost`),
  `AbstractRevenueFunction`, `AbstractDamageFunction`.
