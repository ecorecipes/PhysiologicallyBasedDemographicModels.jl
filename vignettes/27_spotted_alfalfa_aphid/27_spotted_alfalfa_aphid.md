...
# NOTE: Two development rate formulations are used below:
#   1. BriereDevelopmentRate — nonlinear Brière curve for accurate rate-at-T
#   2. LinearDevelopmentRate — linear degree-day approximation for DD accumulation
# Both share the same thermal thresholds (TL, TU). The SAA_A coefficient
# is from the paper's Table 1 but is not used directly by either model;
# it is retained here for reference.
const SAA_A  = 0.008    # development rate coefficient (Table 1, not used in code)
