...
- [Olive Phenology Model](#olive-phenology-model)
  ...
  - [Olive canopy biomass trajectories under the baseline
    scenario](#olive-canopy-biomass-trajectories-under-the-baseline-scenario)
  - [Bloom timing response to
    warming](#bloom-timing-response-to-warming)
...
for ΔT in [0.0, 1.0, 1.8, 2.5, 3.6]
    ...
    push!(warming_levels, ΔT)
    push!(bloom_days, Float64(bloom_day))
...
### Olive canopy biomass trajectories under the baseline scenario

``` julia
fig = Figure(size=(860, 480))
ax = Axis(fig[1, 1],
    xlabel="Day of year",
    ylabel="Dry matter (g/tree)",
    title="Olive canopy and fruit trajectories")
for (i, (name, color)) in enumerate(zip([:leaf, :shoot, :fruit], [:forestgreen, :saddlebrown, :darkorange]))
    lines!(ax, sol_olive.t, stage_trajectory(sol_olive, i); color=color, linewidth=2, label=string(name))
end
axislegend(ax, position=:rt, framevisible=false)
fig
```

<img src="11_olive_climate_files/figure-commonmark/cell-5-output-1.png"
width="860" height="480" />

### Bloom timing response to warming
...
ax = Axis(fig[1, 1],
    xlabel="Warming (°C)",
    ylabel="Bloom day",
    title="Olive bloom advances with warming")
lines!(ax, warming_levels, bloom_days; color=:firebrick, linewidth=3, label="Bloom day")
axislegend(ax, position=:rt, framevisible=false)
fig
```

<img src="11_olive_climate_files/figure-commonmark/cell-6-output-1.png"
