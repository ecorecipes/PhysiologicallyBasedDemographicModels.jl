---
title: "Drosophila suzukii Adult Male Dynamics"
subtitle: "Von Foerster PDE Model with Temperature-Dependent Rates"
author: "Simon Frost"
format:
  html:
    code-fold: true
...
Primary reference: [@Rossini2020DSuzukiiMales].

## Introduction

The spotted-wing drosophila (*Drosophila suzukii*) is a highly invasive
fruit pest native to East Asia. Unlike most *Drosophila* species, females
possess a serrated ovipositor that allows egg-laying into intact,
ripening soft fruit — causing enormous economic damage in berry, cherry,
and grape production across Europe and North America.

Rossini et al. [-@Rossini2020DSuzukiiMales] developed a physiologically
based demographic model for *D. suzukii* adult males using the
**Von Foerster partial differential equation** framework. The PDE tracks
the density of individuals as a function of calendar time $t$ and
physiological age $x$:
...
- $N(t, x)$ is the population density at time $t$ and physiological age $x$,
...
Development rate follows the Brière nonlinear model (Eq. 3 in
Rossini et al. [-@Rossini2020DSuzukiiMales], best-fit parameters):
...
Mortality follows a quadratic function of temperature (Eq. 6),
with a minimum near 15 °C and increasing rates at both cold and
warm extremes:
...
where $t_{ac}$ is physiological age since adult emergence (days),
SR = 0.5 is the sex ratio, $H = 0.585$ eggs/day is the peak
fecundity scaling, $L = 1.0475$ controls the age-dependent decline,
$T_1 = 20.875\,°\text{C}$ is the thermal optimum, and
$T_2 = 8.125\,°\text{C}$ is the thermal half-width. The temperature
component is positive for $T \in (12.75, 29.0)\,°\text{C}$ approximately.
...
expressed in degree-days above the lower threshold ($T_L = 3\,°\text{C}$):
...
`AbstractDevelopmentRate` type rather than using `LinearDevelopmentRate`.
...
#| fig-cap: "Total population dynamics under four discretisation methods."
methods_to_compare = [
...
`PSPMSpecies` PDE approach resolves the full continuous age distribution.
...
This vignette demonstrates two complementary approaches to modeling
*D. suzukii* population dynamics using the Von Foerster PDE framework
from Rossini et al. [-@Rossini2020DSuzukiiMales]:
...
1. **PBDM distributed delay:** Practical for applied pest management.
   ...
   integrates with the broader `PhysiologicallyBasedDemographicModels.jl`
   ecosystem for multi-trophic and spatial extensions.
...
- **Seasonal timing:** Population growth begins in spring as temperatures
  cross the lower developmental threshold ($T_L = 3\,°\text{C}$) and
  peaks in midsummer when the development rate is highest.
