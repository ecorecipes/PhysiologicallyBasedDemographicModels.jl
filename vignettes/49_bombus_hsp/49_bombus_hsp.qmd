---
title: "Genetically Modified Thermal Tolerance in Bombus terrestris"
subtitle: "HSP Promoter Engineering, Pleiotropic Costs, and Pollination Ecosystem Services"
author: "Simon Frost"
format:
  html:
    code-fold: true
...
> **Status — speculative scenario, not a paper reproduction.**
> Unlike most other vignettes in this gallery, this one is *not* a
> reproduction of a published *Bombus* paper. It is a speculative
> scenario that combines (i) established bumblebee colony biology
> with (ii) hypothetical genetically-modified HSP70 promoter
> engineering whose fitness costs are *adapted from Drosophila*
> studies (Hoekstra & Montooth 2013; Silbermann & Tatar 2000;
> Feder 1999). Use it as a methodological demonstration of how the
> framework can host genetic-architecture and stress-physiology
> extensions, not as a quantitative GM-bumblebee assessment.
...
bumblebees have a critical thermal maximum (CT~max~) of ~44 °C (thorax)
and show impaired foraging above 32 °C ambient temperature, cognitive
deficits (learning/memory) at sustained 32 °C exposure, and larval
mortality at 35 °C.
...
enhanced-response variant would upregulate chaperone expression, shifting
the thermal tolerance window upward by 2–4 °C. However, HSP
...
| Genotype | Frequency | CT~max~ shift | Metabolic cost | Fecundity penalty | Longevity penalty |
|----------|-----------|---------------|----------------|-------------------|-------------------|
...
$\Delta$ is the CT~max~ shift for homozygous GM individuals, and $c_m$,
$c_f$, $c_l$ are the metabolic, fecundity, and longevity cost
...
Worker mortality increases quadratically above the stress threshold.
GM bees shift this threshold upward but pay a baseline longevity cost.
...
penalty. The seasonal schedule follows the *B. terrestris* colony
cycle: founding in spring (day 90), ergonomic phase through summer,
...
flowers/hour = 160/day during bloom), and $\beta_p$ is the
pollination efficiency coefficient.
...
the metabolic cost ($c_m$) and fecundity penalty ($c_f$) and compute
the crossover temperature where GM becomes advantageous.
...
3. **Heatwave resilience**: Even at moderate mean temperatures, GM
   ...
   Forager survival and pollination activity during 10-day heat episodes
   are substantially higher for GG genotypes, suggesting GM bees as
   "insurance" against extreme events rather than continuous performers.
...
5. **Allele frequency dynamics**: Under current climate, the $G$ allele
   ...
   frequency, suggesting that prophylactic release would require ongoing
   supplementation until climate conditions favor the modification.
...
- The model uses a simplified ODE for colony dynamics rather than a
  full delay-differential equation with separate caste emergence delays.
- Spatial foraging dynamics (landscape resource availability, flight
  range) are not included.
- The genetic model assumes Hardy–Weinberg equilibrium and no
  genetic drift, which is unrealistic for small wild populations.
