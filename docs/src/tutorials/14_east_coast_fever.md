# East Coast Fever in African Livestock (*Theileria parva*–*Rhipicephalus appendiculatus*–Cattle)
...
# Fitted to development data across East African populations
...
# τ = mean developmental time in degree-days (Brière scale)
# k = Erlang substages (higher k = tighter distribution of development times)
# μ = background mortality rate per degree-day
#
# Stage durations calibrated from Branagan (1973), Randolph & Rogers (1997)
...
through an optimum (~27°C for *R. appendiculatus*), then declining sharply
near the upper lethal limit.
...
### Weather: East African Highlands (Central Kenya)
...
highlands (~1°S, 37°E, ~1800 m elevation) with a mean annual
temperature of ~19°C and moderate seasonal variation.

```julia
# Approximate climate for central Kenya highlands
n_years = 5
n_days = 365 * n_years
...
for d in 1:n_days
    doy = ((d - 1) % 365) + 1
    # Mean ~19°C with bimodal seasonal pattern
    T_base = 19.0
    # Long rains warm-up (March-May) and short rains (Oct-Dec)
...
# Phenology: when does peak adult questing occur each year?
println("\nAnnual adult questing peaks:")
for yr in 1:n_years
    yr_start = (yr - 1) * 365 + 1
    yr_end = yr * 365 + 1
    adult_traj = stage_trajectory(sol_tick, 4)
    yr_slice = adult_traj[yr_start:min(yr_end, length(adult_traj))]
    peak_doy = argmax(yr_slice)
    println("  Year $yr: peak at DOY $peak_doy, N=$(round(yr_slice[peak_doy], digits=0))")
end
```

## Cattle Host Population

The model represents a smallholder cattle herd in the Kenya highlands
with a simple age-structured population. Cattle are modeled with
age-based progression using a flat development rate (1 DD per day).

```julia
# --- Cattle age structure ---
# Development is calendar-time based, not temperature-driven
cattle_dev = LinearDevelopmentRate(0.0, 50.0)

# Herd demographics from Bebe et al. (2003) and Reynolds et al. (1996)
const HERD_SIZE = 200.0  # cattle per farm unit (~2 ha)

cattle_stages = [
    LifeStage(:calf,
              DistributedDelay(10, 365.0;  W0 = 0.20 * HERD_SIZE),
              cattle_dev, 0.0004),    # calf mortality ~15%/yr
    LifeStage(:yearling,
              DistributedDelay(10, 730.0;  W0 = 0.25 * HERD_SIZE),
              cattle_dev, 0.0002),    # yearling mortality ~7%/yr
    LifeStage(:adult,
              DistributedDelay(15, 2190.0; W0 = 0.55 * HERD_SIZE),
              cattle_dev, 0.0001),    # adult mortality ~4%/yr
]
...
println("  Cattle per ha:   ", round(total_population(cattle) / 2.0, digits=1))

# Cattle demographic parameters (Gilioli et al. 2009, Table 1)
...
println("  Net growth rate: ", round((CATTLE_BIRTH_RATE - CATTLE_DEATH_RATE) * 365, digits=3), "/yr")
```

## Tick–Host Attachment

Tick attachment to hosts follows a Monod (saturating) functional
response as in Gilioli et al. (2009): the attachment rate increases
with host density but saturates at high density. This is modeled
with a Holling Type II functional response.

```julia
# Tick attachment parameters (Gilioli et al. 2009, Table 1)
# α = 0.014033 day⁻¹ — attachment rate  [Branagan 1973; Randolph & Rogers 1997]
# λ = 10 cattle ha⁻¹ — semi-saturation  [assumed; Gilioli et al. 2009]
# δ = 0.06487 day⁻¹  — detachment rate  [assumed; Randolph & Rogers 1997]
# Holling Type II: f(H) = a * H / (1 + a * h * H)
# Monod equivalent: f(H) = α * H / (λ + H) → a = α/λ, h = 1/α
...
# Trophic link: ticks feed on cattle
tick_cattle_link = TrophicLink(
    :rhipicephalus_appendiculatus,
    :bos_taurus,
    tick_fr,
    0.02   # low conversion: blood meal sustains reproduction
)
...
molting process so that the emerging nymph is infectious. Similarly,
infected nymphs transmit as adults. Transovarial transmission does
not occur. We model the vector–host disease cycle using
`VectorBorneDisease` and track susceptible, exposed (incubating),
and infectious compartments in both ticks and cattle.
...
```julia
# Cattle: initially 5% prevalence (early epidemic or low endemicity)
cattle_disease = DiseaseState(
    0.95 * HERD_SIZE,   # S: susceptible
    0.05 * HERD_SIZE    # I: infected/infectious
)

# Tick vector: 2% initially infectious
...
println("  Tick vectors — S: $(round(tick_vector.S, digits=0)), " *
...
We simulate 5 years of coupled tick–cattle–disease dynamics. The
effective bite rate depends on tick density and cattle density
through the functional response, and seasonal temperature variation
drives tick population oscillations that create windows of intense
transmission.

```julia
# --- Coupled simulation: 5 years ---
const BITE_RATE_BASE = 0.014  # baseline feeding events per tick per day
n_sim = 365 * n_years
...
# Tick population state (simplified logistic for total abundance)
tick_N = Float64(TICK_INITIAL_POP)
const TICK_CARRYING_CAPACITY = 30000.0  # ticks per ha
...
for day in 1:n_sim
    ...
    # Seasonal mortality pulse (dry season stress)
    ...
    # Effective bite rate: depends on cattle density via functional response
    ...
    # Cattle demographics: births and natural deaths
...
Tick activity in the East African highlands peaks during and just
after the rainy seasons, creating two annual waves of transmission.
The long rains (March–May) produce the larger tick cohort and
the main ECF transmission season.

```julia
# Monthly summary for year 3 (quasi-equilibrium)
...
high infection rates) and **epidemic outbreaks** in naïve herds.
We compare three scenarios:

1. **Endemic**: continuous low-level transmission in a partially immune herd
2. **Epidemic**: *T. parva* introduced into a fully susceptible herd
3. **Post-acaricide**: tick control relaxed after suppression, causing rebound

```julia
function simulate_scenario(;
    S0::Float64, I0::Float64, R0_init::Float64,
    vec_inf_frac::Float64, n_days::Int, label::String)
...
stability**: a state where high seroprevalence maintains herd immunity
while clinical disease and mortality remain low. This requires
maintaining tick populations above a minimum threshold to sustain
natural immunization.

```julia
# Critical cattle density from Gilioli et al. (2009) Eq. (4)
# h_crit = λ * μ * (δ + η) / [a * α * (φ - η) - μ * (δ + η)]
...
a_resist = 1.0  # no resistance (European cattle)
...
println("  (Below this density, ticks and ECF disappear)")

# R₀ threshold for tick persistence (Eq. 7)
...
depends on temperature through its effects on tick development rate,
population density, and the extrinsic incubation period. We calculate
R₀ across a range of mean temperatures representing different
altitudinal zones in East Africa.

```julia
# Vector-borne R₀ (Ross-Macdonald framework adapted for ticks):
# R₀ = m × a² × β_vh × β_hv / (μ_v × (γ_h + μ_h))
# × 1 / (1 + μ_v × EIP)
#
# where:
#   m = vector-to-host ratio
#   a = bite rate per tick per day
#   μ_v = vector (tick) mortality rate
...
function compute_R0_ecf(T_mean::Float64; m::Float64=100.0)
    ...
    # Temperature-dependent EIP (faster at higher T)
    eip = max(7.0, 21.0 * development_rate(tick_dev, 25.0) / max(0.001, dr))
    eip = min(eip, 60.0)

    # Tick survival: higher mortality at temperature extremes
    mu_v = 0.035 + 0.002 * max(0.0, T_mean - 30.0) + 0.002 * max(0.0, 15.0 - T_mean)

    # Vector-to-host ratio scales with temperature (development rate effect)
    m_eff = m * dr / max(0.001, development_rate(tick_dev, 25.0))

    a = BITE_RATE_BASE  # feeding rate per tick per day
    beta_vh = ecf.β_vh
    beta_hv = ecf.β_hv
    gamma_h = ecf.γ_h
    mu_h = ecf.μ_h
    ...
    # Ross-Macdonald R₀
    R0 = m_eff * a^2 * beta_vh * beta_hv /
         (mu_v * (gamma_h + mu_h) * (1.0 + mu_v * eip))
    return R0
end
...
We examine R₀ sensitivity to a +2°C warming scenario.
...
for (alt, T_now) in [(2400, 16.0), (2000, 18.0), (1800, 20.0),
    ...
    R0_warm = compute_R0_ecf(T_now + 2.0)
    change = R0_now > 0 ? (R0_warm - R0_now) / R0_now * 100 : Inf
    println("  $(rpad(alt, 9))| $(rpad(T_now, 10))" *
            "| $(rpad(round(R0_now, digits=2), 7))" *
            "| $(rpad(T_now + 2.0, 7))" *
            "| $(rpad(round(R0_warm, digits=2), 8))" *
...
Following Gilioli et al. (2009), we evaluate three integrated tick
management strategies by modifying model parameters:

1. **Acaricide dipping**: increases attached tick mortality η
2. **Pasture rotation**: reduces host availability (lower effective H)
3. **Resistant breeds**: reduces attachment efficiency (a < 1)

```julia
println("\nTick control strategy evaluation:")
println("="^70)

# Baseline R₀ at T=20°C
R0_baseline = compute_R0_ecf(20.0)
println("Baseline R₀ at 20°C: $(round(R0_baseline, digits=2))")

# Strategy 1: Acaricide dipping — reduce tick survival
println("\n1. Acaricide dipping (increasing η):")
for eta_mult in [1.0, 1.5, 2.0, 3.0, 5.0]
    # Higher η reduces m (fewer ticks survive to bite)
    R0_acar = R0_baseline / eta_mult
    println("   η × $(rpad(eta_mult, 4)) → R₀ = $(round(R0_acar, digits=2))" *
            "$(R0_acar < 1.0 ? "  ✓ disease eliminated" : "")")
end

# Strategy 2: Pasture rotation — reduce effective host density
println("\n2. Pasture rotation (reducing H₀):")
for h_frac in [1.0, 0.7, 0.5, 0.3, 0.1]
    # Functional response reduces feeding success
    attach_frac = functional_response(tick_fr, HERD_SIZE/2.0 * h_frac) /
                  max(0.001, functional_response(tick_fr, HERD_SIZE/2.0))
    R0_rot = R0_baseline * attach_frac^2  # a² in R₀ formula
    println("   H × $(rpad(h_frac, 4)) → R₀ = $(round(R0_rot, digits=2))" *
            "$(R0_rot < 1.0 ? "  ✓ disease eliminated" : "")")
end

# Strategy 3: Resistant breeds — reduce attachment coefficient
println("\n3. Breed resistance (reducing a):")
for a_coeff in [1.0, 0.7, 0.5, 0.3, 0.1]
    R0_resist = R0_baseline * a_coeff^2
    println("   a = $(rpad(a_coeff, 4)) → R₀ = $(round(R0_resist, digits=2))" *
            "$(R0_resist < 1.0 ? "  ✓ disease eliminated" : "")")
end
```
...
- O'Callaghan, C.J., Medley, G.F., Peter, T.F., Perry, B.D. (1998).
  Investigating the epidemiology of heartwater (*Cowdria ruminantium* infection)
  by means of a transmission dynamics model. *Parasitology*, 117, 49–61.
- Randolph, S.E., Rogers, D.J. (1997). A generic population model for the
  African tick *Rhipicephalus appendiculatus*. *Parasitology*, 115, 265–279.
