# Aedes albopictus Invasion Risk in Europe (Pasquali et al. 2020)
...
2. **Photoperiod-driven egg diapause**: Short days trigger a fraction of
   eggs to enter dormancy, enabling overwintering
...
Ponti, L., Chiari, M., Sperandio, G., & Gilioli, G. (2020).
*Development and calibration of a model for the potential establishment
and impact of Aedes albopictus in Europe.* Acta Tropica, 202, 105228.
...
Adult mortality (both sexually immature and reproductive) is a
constant estimated at 0.067/day (Section 2.1.3).
...
diapause**: as day length decreases in autumn, an increasing fraction
of eggs enter a dormant state that enables winter survival. The
diapause induction function follows Lacour et al. (2015):

$$\text{diap}(D) = \frac{1}{1 + e^{3.04(D - 12.62)}}$$

where *D* is day length in hours (Eq. 15). Diapause eggs accumulate
from July 1 to December 31 when photoperiod falls below 15 hours
(D_max). They break diapause when day length reaches 11.25 hours
(D_min) **and** the weekly minimum temperature average exceeds 12.5°C
(Section 2.2).
...
Day length variation drives the timing of diapause entry and exit
at different latitudes. Southern locations have shorter winters but
weaker photoperiod signals.
...
photoperiod shortens, a fraction of eggs enters the diapause storage.
In spring, stored diapause eggs are released back into the egg stage.
...
reproduction callback. The simulation runs for 3 years to reach
a stable annual pattern (as per Pasquali et al. 2020, who used
10–20 year runs).
...
for (name, sol, col) in [("Rome", rome_sol, :firebrick),
    ...
    lines!(ax6, sol.t[1:end-1], cdd, label=name, linewidth=2, color=col)
end
...
The diapause mechanism is the critical determinant of whether
*Ae. albopictus* can establish at a given latitude. Diapausing eggs
must survive winter cold and retain enough numbers to recolonize in
spring.
...
*Ae. albopictus* is a competent vector for multiple arboviruses.
The combination of population abundance and seasonal activity period
...
1. **Diapause determines the northern range limit**: The photoperiod-driven
   egg diapause mechanism enables *Ae. albopictus* to persist through
   Mediterranean and central European winters. At latitudes above ~50°N,
   cold-induced mortality of diapausing eggs and insufficient summer
   degree-days prevent stable establishment.
...
4. **Density-dependent regulation matters**: Larval competition
   ...
   explosions and produces realistic egg counts matching field
   ovitrap data from the Bolzano Province.

5. **Climate change implications**: Warmer temperatures will both
   extend the activity season northward and increase degree-day
   accumulation, potentially shifting the establishment boundary
   from ~48°N to ~52°N. Combined with increased diapause egg
   survival during milder winters, this creates expanding risk
   zones for vector-borne disease outbreaks in currently temperate
   European regions.
...
6. **Mechanistic vs. correlative models**: Unlike species distribution
   ...
   simulates the biological processes that determine persistence.
   This enables predictions in areas where the mosquito has not yet
...
|-----------|-------|--------|-------------|
| `EGG_DEV_A` | 4.16657×10⁻⁵ | Table 1, Eq. 9 | Egg development rate coefficient |
| `EGG_DEV_TSUP` | 37.3253 °C | Table 1, Eq. 9 | Egg upper thermal limit |
| `LARVA_DEV_A` | 8.604×10⁻⁵ | Table 1, Eq. 10 | Larval Brière coefficient |
| `LARVA_DEV_TINF` | 8.2934 °C | Table 1, Eq. 10 | Larval lower thermal limit |
| `LARVA_DEV_TSUP` | 36.0729 °C | Table 1, Eq. 10 | Larval upper thermal limit |
| `PUPA_DEV_A` | 3.102×10⁻⁴ | Table 1, Eq. 10 | Pupal Brière coefficient |
| `PUPA_DEV_TINF` | 11.9433 °C | Table 1, Eq. 10 | Pupal lower thermal limit |
| `PUPA_DEV_TSUP` | 40.0 °C | Table 1, Eq. 10 | Pupal upper thermal limit |
| `IMMADULT_DEV_A` | 1.812×10⁻⁴ | Table 1, Eq. 10 | Immature adult Brière coefficient |
| `IMMADULT_DEV_TINF` | 7.7804 °C | Table 1, Eq. 10 | Immature adult lower thermal limit |
| `IMMADULT_DEV_TSUP` | 35.2937 °C | Table 1, Eq. 10 | Immature adult upper thermal limit |
| `ADULT_DEV_RATE` | 0.015 /day | Section 2.1.3 | Reproductive adult development rate |
| `EGG_MORT_A,B,C` | 0.002869, −0.1417, 2.1673 | Table 2, Eq. 11 | Egg proportional mortality coefficients |
| `LARVA_MORT_A,B,C` | 0.002793, −0.1255, 1.5768 | Table 2, Eq. 11 | Larval proportional mortality coefficients |
| `PUPA_MORT_A,B,C` | 0.003289, −0.1437, 1.6197 | Table 2, Eq. 11 | Pupal proportional mortality coefficients |
| `EGG_MORT_ALOW,AHIGH` | 0.05, 0.018 | Table 3, Eq. 12 | Egg mortality rate slopes |
| `EGG_MORT_TLOW,THIGH` | 12.0, 30.5 °C | Table 3, Eq. 12 | Egg mortality rate breakpoints |
| `LARVA_MORT_ALOW,AHIGH` | 0.2, 0.15 | Table 3, Eq. 12 | Larval mortality rate slopes |
| `LARVA_MORT_TLOW,THIGH` | 17.0, 30.5 °C | Table 3, Eq. 12 | Larval mortality rate breakpoints |
| `PUPA_MORT_ALOW,AHIGH` | 0.1, 0.2 | Table 3, Eq. 12 | Pupal mortality rate slopes |
| `PUPA_MORT_TLOW,THIGH` | 17.0, 37.5 °C | Table 3, Eq. 12 | Pupal mortality rate breakpoints |
| `ADULT_MORTALITY` | 0.067 /day | Section 2.1.3 | Adult mortality rate (constant) |
| `FECUND_A` | 0.0032 | Table 5, Eq. 14 | Fecundity Brière coefficient |
| `FECUND_TINF` | 16.24 °C | Table 5, Eq. 14 | Fecundity lower thermal limit |
| `FECUND_TSUP` | 35.02 °C | Table 5, Eq. 14 | Fecundity upper thermal limit |
...
| `DIAP_DMAX` | 15.0 h | Section 2.2 | Maximum day length for diapause |
| `DIAP_DMIN` | 11.25 h | Section 2.2 | Minimum day length for diapause break |
| `DIAP_TBREAK` | 12.5 °C | Section 2.2 | Temperature threshold for diapause break |
| `DIAP_MORT_A` | 0.05301 | Eq. 16, Thomas et al. (2012) | Diapause egg mortality coefficient |
| `DIAP_MORT_B` | −0.1649 | Eq. 16, Thomas et al. (2012) | Diapause egg mortality exponent |

## References

- Pasquali, S., Mariani, L., Calvitti, M., Moretti, R., Ponti, L.,
  Chiari, M., Sperandio, G., & Gilioli, G. (2020). Development and
  calibration of a model for the potential establishment and impact
  of *Aedes albopictus* in Europe. *Acta Tropica*, 202, 105228.
- Lacour, G., Chanaud, L., L'Ambert, G., & Hance, T. (2015).
  Seasonal synchronization of diapause phases in *Aedes albopictus*
  (Diptera: Culicidae). *PLoS ONE*, 10(12), e0145311.
...
- Brière, J. F., Pracros, P., Le Roux, A. Y., & Pierre, J. S. (1999).
