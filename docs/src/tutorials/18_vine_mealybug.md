# Vine Mealybug Biocontrol: Refuge Effects and Climate (Gutierrez et al. 2008)
...
function simulate_vmb_biocontrol(;
    ...
    # State variables
    vmb_exposed = initial_vmb * (1.0 - base_refuge)
    vmb_refuged = initial_vmb * base_refuge
    ap_adults = use_ap ? 10.0 : 0.0
    la_adults = use_la ? 5.0 : 0.0
    cm_adults = use_cm ? 2.0 : 0.0
    vmb_parasitized_ap = 0.0
    vmb_parasitized_la = 0.0

    # Output trajectories
    vmb_total_hist = Float64[]
    vmb_exposed_hist = Float64[]
    vmb_refuged_hist = Float64[]
    ap_hist = Float64[]
    la_hist = Float64[]
    cm_hist = Float64[]
    parasitism_ap_hist = Float64[]
    parasitism_la_hist = Float64[]

    for day in 1:n_days
        T = daily_temps[day]

        # Degree-days for each species
        dd_vmb = max(0.0, T - VMB_T_LOWER)
        dd_ap  = max(0.0, T - AP_T_LOWER)
        dd_la  = max(0.0, T - LA_T_LOWER)
        dd_cm  = max(0.0, T - CM_T_LOWER)

        # Temperature-dependent refuge
        H = refuge_fraction(T, base_refuge)

        # VMB reproduction (temperature-scaled)
        ...
        if ap_adults > 0 && vmb_exposed > 0
            ...
            vmb_parasitized_ap += parasitized_ap
        end

        # --- Parasitism by L. abnormis ---
        parasitized_la = 0.0
        if la_adults > 0 && vmb_exposed > 0
            ...
            vmb_parasitized_la += parasitized_la
        end
        ...
        # A. pseudococci: emergence from parasitized mealybugs
        ap_emergence = vmb_parasitized_ap * (dd_ap / 519.0) * 0.3
        vmb_parasitized_ap *= (1.0 - dd_ap / 519.0)
        vmb_parasitized_ap = max(0.0, vmb_parasitized_ap)
        ap_mortality = 0.002 * dd_ap
        ap_adults = max(0.0, ap_adults * (1.0 - ap_mortality) + ap_emergence)

        # L. abnormis: emergence
        la_emergence = vmb_parasitized_la * (dd_la / 652.0) * 0.3
        vmb_parasitized_la *= (1.0 - dd_la / 652.0)
        vmb_parasitized_la = max(0.0, vmb_parasitized_la)
        la_mortality = 0.002 * dd_la
        la_adults = max(0.0, la_adults * (1.0 - la_mortality) + la_emergence)

        # C. montrouzieri: numerical response to prey
        cm_reproduction = cm_adults > 0 ? 0.01 * vmb_total * (dd_cm / DD_PER_GEN_CM) : 0.0
        cm_mortality = 0.003 * dd_cm
        cm_adults = max(0.0, cm_adults * (1.0 - cm_mortality) + cm_reproduction * 0.1)

        # VMB background mortality
        ...
        push!(vmb_total_hist, vmb_exposed + vmb_refuged)
        push!(vmb_exposed_hist, vmb_exposed)
        push!(vmb_refuged_hist, vmb_refuged)
        push!(ap_hist, ap_adults)
        push!(la_hist, la_adults)
        push!(cm_hist, cm_adults)
        push!(parasitism_ap_hist, prop_ap)
        push!(parasitism_la_hist, prop_la)
    end

    return (
        vmb_total = vmb_total_hist,
        vmb_exposed = vmb_exposed_hist,
        vmb_refuged = vmb_refuged_hist,
        ap = ap_hist,
        la = la_hist,
        cm = cm_hist,
        parasitism_ap = parasitism_ap_hist,
        parasitism_la = parasitism_la_hist,
    )
end
```
