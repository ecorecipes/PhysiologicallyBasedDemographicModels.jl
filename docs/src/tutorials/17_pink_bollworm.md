...
function emergence_fraction(dd::Real)
    # Logistic emergence centered at DD_EMERGENCE_MEAN
...
```julia
# --- Simulate one season at Phoenix, AZ ---
function simulate_pbw_season(weather; N_diapause=2.0)
    # Track key variables through the season
    n = length(weather.days)
    cum_dd_pbw = zeros(n)
    cum_dd_cotton = zeros(n)
    cold_dd_cum = zeros(n)
    adults_emerged = zeros(n)
    egg_density = zeros(n)
    larva_density = zeros(n)
    diapause_larvae = zeros(n)

    # Accumulate degree-days from Jan 1
    for d in 2:n
        T_mean = weather.days[d].T_mean
        cum_dd_pbw[d] = cum_dd_pbw[d-1] + degree_days(pbw_dev, T_mean)
        cum_dd_cotton[d] = cum_dd_cotton[d-1] + degree_days(cotton_dev, T_mean)

        # Cold DD accumulation (winter months)
        T_min = weather.days[d].T_min
        cold_dd_cum[d] = cold_dd_cum[d-1] + daily_cold_dd(T_min)
    end

    # Compute overwintering survival
    # Cold accrues from previous fall — use cumulative cold through spring
    spring_cold = cold_dd_cum[90]  # Through end of March
    overwinter_surv = cold_survivorship(spring_cold)
    surviving_diapause = N_diapause * overwinter_surv

    # Adult emergence (spring)
    for d in 1:n
        frac = emergence_fraction(cum_dd_pbw[d])
        adults_emerged[d] = surviving_diapause * frac
    end

    # Simplified generation tracking
    # Generation 1 oviposition begins when cotton fruit available AND adults present
    for d in 1:n
        T_mean = weather.days[d].T_mean
        dl = weather.days[d].photoperiod

        # Cotton fruit availability (sigmoid onset at DD_FIRST_SQUARE)
        fruit_avail = 1.0 / (1.0 + exp(-0.01 * (cum_dd_cotton[d] - DD_FIRST_SQUARE)))

        # Daily oviposition by emerged adults
        phi_T = temperature_fecundity_index(T_mean)
        daily_eggs = adults_emerged[d] * PBW_FECUNDITY_MAX / DD_ADULT *
                     degree_days(pbw_dev, T_mean) * PBW_SEX_RATIO * phi_T * fruit_avail
        egg_density[d] = daily_eggs

        # Larval density builds with a lag (generation time ≈ DD_EGG + DD_LARVA)
        gen_dd = DD_EGG + DD_LARVA
        lag_days = max(1, round(Int, gen_dd / max(1.0, degree_days(pbw_dev, T_mean))))
        if d + lag_days <= n
            larva_density[d + lag_days] += daily_eggs * 0.7  # 70% egg-to-larva survival
        end

        # Diapause induction in fall
        diap_frac = diapause_fraction(dl, T_mean)
        if diap_frac > 0.0 && d > 200  # After mid-July
            diapause_larvae[d] = larva_density[d] * diap_frac
        end
    end
